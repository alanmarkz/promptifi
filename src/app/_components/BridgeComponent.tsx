...
const BridgeComponent = ({ transaction }: { transaction: any }) => {
  ...
  const sourceAmount =
    Number(transaction.estimation?.srcChainTokenIn?.amount ?? 0) /
    10 ** Number(transaction.estimation?.srcChainTokenIn?.decimals);

  const intermediateAmount =
    Number(transaction.estimation?.srcChainTokenOut?.amount ?? 0) /
    10 ** Number(transaction.estimation?.srcChainTokenOut?.decimals);

  const destinationAmount =
    Number(transaction.estimation?.dstChainTokenOut?.amount ?? 0) /
    10 ** Number(transaction.estimation?.dstChainTokenOut?.decimals);
      ...
      <div className="flex items-center justify-between space-x-4">
        ...
        >
          <div className="text-xs text-gray-500">
            {transaction.estimation.srcChainTokenIn.name}
          </div>
          <div className="font-bold text-lg">
            {Number(sourceAmount.toFixed(4))}{" "}
            <span className="text-blue-500">
              {transaction.estimation.srcChainTokenIn.symbol}
            </span>
          </div>
        </motion.div>
        {/* Arrow */}

        {transaction.estimation.srcChainTokenOut && (
          <>
            ...
            >
              <div className="text-xs text-gray-500">
                {transaction.estimation.srcChainTokenOut.name}
              </div>
              <div className="font-bold text-lg">
                {Number(intermediateAmount.toFixed(4))}{" "}
                <span className="text-blue-500">
                  {transaction.estimation.srcChainTokenOut.symbol}
                </span>
              </div>
            </motion.div>
          </>
        ...
        >
          <div className="text-xs text-gray-500">
            {transaction.estimation.dstChainTokenOut.name}
          </div>
          <div className="font-bold text-lg">
            {Number(destinationAmount.toFixed(4))}{" "}
            <span className="text-blue-500">
              {transaction.estimation.dstChainTokenOut.symbol}
            </span>
          </div>
        </motion.div>
      </div>
